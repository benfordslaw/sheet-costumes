  ...
  putChar: function(c, args) {
    ...
    for (var i = 0; i < content.length; i += 2) {
      ...
      if (digit == " R") {
        ...
        var x = randomGaussian(digit[0].charCodeAt(0) - ordR, noiseVal / 1.5);
        var y = randomGaussian(digit[1].charCodeAt(0) - ordR, noiseVal / 1.5);
          P5.vertex(x, y);
        }
      }
      P5.endShape();
  ...
  putText: function(wordStartTime, activeWord, s, args) {
    ...
    for (var i = 0; i < s.length; i++) {
      if(activeWord){
        ...
        if(s[i] == ' '){
          var x = randomGaussian(0, 3);
          var y = randomGaussian(0, 1.5);
          P5.translate(x, y);
          xShift += x;
          yShift += y;
        }
        var x = P5.animatedHershey.putChar(s[i], args);
        var y = randomGaussian(0, noiseVal / 4);
        P5.translate(x, y);
        xShift += x;
        yShift += y;
      }
